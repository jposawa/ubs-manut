...
export const CadAltServicos = () => {
  const { id } = useParams();
  const [dadosServicos, defineDadosServicos] = React.useState();
  ...
  React.useEffect(() => {
    ...
    if (id) {
      axios.get(URL_SERVICOS, {
         ...
         */
        defineDadosServicos(response.data[0]);
  ...
  const salvarDadosServicos = (dados) => {
    const usuarioSessao = JSON.parse(sessionStorage.getItem('ubs-usuario'));
  ...
  return (
    <>
      <div className="tituloPaginas">Cadastro de Serviços</div>
      <form className={styles.formPrincipal} onSubmit={salvarDadosServicos}>
        <div className='containerInputs'>
          <p>
            <Input label="Descrição:" type="text" name="descricao" defaultValue={dadosServicos?.descricaoServico} size="40" required key={renderKey} maxLength="50"/>
          </p>
          <p>
            <Input label="Codigo Interno:" type="text" name="codigoInterno" defaultValue={dadosServicos?.codigoInterno} size="10" key={renderKey} maxLength="10" />
          </p>
          <p>
            <Input label="Valor do serviço:" type="tel" maxLength="12" name="valorCobrado" defaultValue={dadosServicos?.valorServico} size="12" required onChange={formatarValor} key={renderKey} />
          </p>
        </div>
